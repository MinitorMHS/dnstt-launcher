...
$defaultDnsData = @(
    @{ Name = "Default Server(Google)"; Address = "8.8.8.8" },
...
$linkLabel.Add_LinkClicked({ try { Start-Process "https://t.me/minitor" } catch {} })
...
function Show-SettingsWindow {
    ...
    $setForm.Size = New-Object System.Drawing.Size(340, 280)
    ...
    $btnSave = New-Object System.Windows.Forms.Button; $btnSave.Text = "Save Settings"; $btnSave.Location = '200,195'; $btnSave.Size = '100,30'
    $setForm.Controls.AddRange(@($lblDom, $txtDom, $lblKey, $txtKey, $lblPort, $txtPort, $btnSave))
    $setForm.AcceptButton = $btnSave

    $btnSave.Add_Click({
        $d = $txtDom.Text.Trim(); $k = $txtKey.Text.Trim(); $p = $txtPort.Text.Trim()
        if (-not $d -or -not $k -or -not $p) { [System.Windows.Forms.MessageBox]::Show("All fields are required.", "Error"); return }
...
function Update-Buttons($isRunning) {
    if ($isRunning) {
        ...
        $status.Text = "Running: $($combo.Text)"; $status.ForeColor = 'Green'
    } else {
